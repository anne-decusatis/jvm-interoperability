// https://github.com/pfn/kotlin-plugin/blob/master/src/main/scala/KotlinCompile.scala
import java.io.File
...
//    KotlinReflection.fromClasspath(cp))
  ...
  def compile(options: Seq[String],
    ...
    val stub = KotlinStub(s, KotlinReflection.fromClasspath(compilerClasspath))
...
//      (d, (d ** javaFiles).get)) filter (_._2.nonEmpty)
    if (kotlinSources.isEmpty) {
      ...
      args.freeArgs = kotlinSources.map(_.getAbsolutePath).asJava
      args.noStdlib = true
...
case class KotlinStub(s: TaskStreams, kref: KotlinReflection) {
  ...
  // https://github.com/pfn/kotlin-plugin/issues/23#issuecomment-390701429
  def compilerArgs = {
